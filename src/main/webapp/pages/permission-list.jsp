...
<%--												<a href="${pageContext.request.contextPath}/permission/deletePermission.do?id=${p.id}" class="btn bg-olive btn-xs">删除权限</a>--%>
												<a  href="javascript:void(0)"; class="btn bg-olive btn-xs" onclick="deletePermission(${p.id})" >删除权限</a>
											</td>
										</tr>
									</c:forEach>
		...
		<script>
			...
			function deletePermission(id){
				// console.log("当前点击对象的id值------"+id);
				// alert("点击的框的id---"+id)
				<%--$.get("${pageContext.request.contextPath}/deletePermission/"+id,function(info){--%>
					$.post("${pageContext.request.contextPath}/deletePermission/"+id,{_method:"delete"},function(map){
					if(map.info == "success"){
						alert("删除成功")
						window.location.replace("${pageContext.request.contextPath}/permissionList");
					}else{
						alert("删除失败")
					}
				});
			...
			$(document).ready(function() {
				$(".refresh").click(function(){
					window.location.href="${pageContext.request.contextPath}/permissionList";
				});
				// 选择框
				$(".select2").select2();
